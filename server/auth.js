...
export const authenticate = async function(req, fromRefeshTokenCookie = false) {
	...
	if (fromRefeshTokenCookie) {
		...
		if (!token) throw new AuthenticationError('Auth error: Refresh token not found!');
	} else {
		// extract jwt access token from HTTP header
		const authorization = req.headers['authorization'];
		if (!authorization) throw new AuthenticationError('Auth error: Missing authorization header!');

		token = authorization.split(' ')[1];
		if (!token) throw new AuthenticationError('Auth error: Invalid authorization header!');
	}
	...
	if (!user) throw new AuthenticationError('Auth error: user id does not exist!');
